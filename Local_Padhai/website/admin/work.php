			...
			<?php
				...
				if($result->num_rows>0)
				{
					echo '
							<table border="1">
								<tr>
									<h2>List of Requesters and Technicians</h2>
								</tr>
								<tr>
									<th>
										<h2>Sno.</h2>
								...
								while($row = $result->fetch_assoc())
								{
									echo '<tr>';
										...
										echo '<td><h3>'.$row["date"].'</h3></td>';
										echo '<td>
										<form action="" method="post">
										<input type="hidden" name="id" value="'.$row["srequestid"].'">
										<button style="background-color: #ff4d4d; type="submit" name="done">Work Done</button>
										</form>
										</td>';
									echo '</tr>';
				...
				if(isset($_POST['done']))
				{
					$sql = "DELETE FROM  `allocation` WHERE srequestid={$_POST['id']}";
					if($conn->query($sql))
					{
